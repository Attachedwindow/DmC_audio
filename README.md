...
我将原始Main_PC.apk保存在了`DmC_audio\datas`下。建议备份原始文件
...
```
models\quickbms_win\quickbms.exe models\Scripts\DmC_APK_to_BNK.bms datas\Main_PC.apk datas\output\Extracted_BNK

models\quickbms_win\quickbms.exe models\Scripts\DmC_BNK_to_WEM.bms datas\output\Extracted_BNK datas\output\Extracted_WEM
```
...
```
python models\Scripts\replace_wem_in_bnk.py datas\output\Extracted_BNK\5809be10.bnk datas\1.wem 99247340 datas\output\Modified_BNK\5809be10.bnk

python models\Scripts\replace_wem_in_bnk.py datas\output\Modified_BNK\5809be10.bnk datas\2.wem 361484552 datas\output\Modified_BNK\5809be10.bnk
```
...
>
> 运行一次后所有音频失效：与Wwise Vorbis 的私有声道布局字段有关，这个不是很懂 → 解决：在替换过程中自动进行检查修改，修改过程能在控制台看到
...
```
models\quickbms_win\quickbms.exe -w -r models\Scripts\DmC_APK_to_BNK.bms datas\Main_PC.apk datas\output\Modified_BNK
```

如果新 BNK 变大，使用 Reimport2，即（没测试过能不能用）

```
models\quickbms_win\quickbms.exe -w -r -r models\Scripts\DmC_APK_to_BNK.bms datas\Main_PC.apk datas\output\Modified_BNK
```
